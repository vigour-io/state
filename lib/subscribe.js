...
module.exports = function (target, subs, update, tree) {
  ...
  // need to add number or string depending on what your expecting --- this way we get optmizations from v8
  // how do we know what its going to be ???? -- maybe specifiy??? stamp structure ???
  diff(target, subs, update, tree, 0, tree, subs)
  return tree
}
...
function struct (key, target, pSubs, subs, update, tree, stamp, rTree, rSubs, force) {
  // findCullprit('struct', key, target, pSubs, subs, update, tree, stamp, rTree, rSubs, force)
  var keyTarget = target[key]
  var treeKey = tree[key]
  if (keyTarget && keyTarget.val !== null) {
    let leafStamp = generateStamp(keyTarget, treeKey, stamp, rTree, rSubs)
    if (!treeKey) {
      ...
      // (target, subs, update, tree, stamp, rTree, rSubs, lstamp)
