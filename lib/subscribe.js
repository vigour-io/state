...
module.exports = function (target, subs, update, tree, stampForce) {
  if (!tree) { tree = {} }
  target.on('subscription', function (data, stamp) {
    diff(target, subs, update, tree, stamp, tree, subs)
  })
  target._subscriptions = true
  // the creation is a bit dangerous (no close event)
  diff(target, subs, update, tree, stampForce || target._lstamp || vstamp.create(), tree, subs)
  return tree
}

function diff (target, subs, update, tree, stamp, rTree, rSubs, force) {
  for (var key in subs) {
...
function item (key, target, pSubs, subs, update, tree, stamp, rTree, rSubs, force) {
  if (key !== 'val' && key !== '$') {
    if (key === 'parent') {
      parent(target, pSubs, subs, update, tree, stamp, rTree, rSubs)
    } else if (key === '$root') {
...
function parent (target, pSubs, subs, update, tree, stamp, rTree, rSubs) {
  if (!tree.$p) {
    console.log('lets go more subs going on', target)
    console.log('CREATE .p$', tree)
    tree.$p = subs
    if (subs === true) {
      subs = pSubs.parent = { val: true }
    } else {
      for (let key in subs) {
        if (key !== 'val') {
    ...
    compositeRoot(target, tree, update, stamp, PARENT, rTree, rSubs)
    computeParent(target, tree, stamp, rTree, rSubs)
    tree.$ = generateStamp(target, tree, stamp, rTree, rSubs)
    if (update && tree.$p.$) {
      // not init is wrong... also want info in update from whom your getting this update e.g. key -- parent
      // key of subs
      update.call(target, INIT, stamp, pSubs, tree)
    }
  } else if (update && stamp === tree.$p.$) {
...
function parentStamp (target, tree, stamp, rTree, rSubs) {
  var subs = tree.$p
  var rstamp
  for (let skey in subs) {
    if (skey !== '$') {
      console.log('??????', skey, tree, target)
      // wrong tree need one up tree very annoying!
      item(skey, target, subs, subs[skey], updateStamp, rTree, stamp, rTree, rSubs, tree.$p.$ || true)
      if (subs[skey].$) {
        console.log('hello?')
        if (!rstamp) {
          rstamp = subs[skey].$
  ...
  if (rstamp) {
    console.log('XXXX yo yo yo ?????')
    subs.$ = rstamp
    return rstamp
  }
}

function computeParent (target, cTree, event, rTree, rSubs, istamp) {
  let pStamp = parentStamp(target, cTree, event, rTree, rSubs)
  if (pStamp) {
    if (!istamp) {
      istamp = pStamp
    } else {
    ...
    if (tree.$c.$ !== stamp) {
      ...
      for (let key in tree.$c) {
        if (key !== '$' && key !== '$$') {
          ...
          if (cTree.$p) {
            istamp = computeParent(target, cTree, stamp, rTree, rSubs, istamp)
          }
          if (tree[key].$c) {
