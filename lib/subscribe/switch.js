...
module.exports = function switchStruct (target, pSubs, subs, update, tree, stamp) {
  const map = subs.map
  var switchTree = tree.$switch
  var keyTree
  var changed
  const switchKey = map(target, stamp, subs, tree)
  ...
  if (!switchTree) {
    ...
    if (switchTree.$ !== switcher || switchTree.$ref !== target) {
      ...
      if ('$' in switchTree) {
        ...
        if ('$remove' in subs) {
          remove('$switch', void 0, switchTree.$, update, tree, switchTree, stamp)
        }
      }
      switchTree = tree.$switch = { _p: tree, _key: '$switch' }
      switchTree.$ref = target
