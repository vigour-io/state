...
module.exports = function root (target, subs, update, tree, stamp, force) {
  ...
  if (target && (!('val' in target) || target.val !== null)) {
    ...
    return diff(target.getRoot(), subs, update, rootTree, stamp, false, force)
  } else if ('$remove' in subs) {
    diff(target, subs, update, rootTree, stamp, false, force)
