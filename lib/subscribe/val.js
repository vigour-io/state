...
exports.update = function updateState (target, travelTarget, subs, update, tree, stamp, force) {
  if ('val' in subs) {
    if (subs.val === true) {
  ...
  diff(travelTarget, subs, update, tree, stamp, false, force)
}

exports.create = function create (target, travelTarget, subs, update, tree, stamp, force) {
  if ('val' in subs) {
    const val = target.val
    if (val && typeof val === 'object' && val.isBase) {
