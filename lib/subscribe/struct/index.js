...
// function bla (key, target, pSubs, subs, update, tree, stamp, rTree, rSubs, force) {
...
//   if ('val' in subs) {
//     bla(key, target, pSubs, subs, update, tree, stamp, rTree, rSubs, force)
//   }
// }

module.exports = function struct (key, target, pSubs, subs, update, tree, stamp, rTree, rSubs, force) {
  // rSubs seems totally useless move it gone! also rtree is not rly nessecary anymore since we have _p
  const keyTarget = target && target[key]
  ...
  if (keyTarget && keyTarget.val !== null) {
    ...
    if (!treeKey) {
      ...
      if ((force || treeKey.$) !== leafStamp) {
        treeKey.$ = leafStamp
        if (subs.val === true || subs.val && force) {
