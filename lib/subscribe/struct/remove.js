...
module.exports = function structRemove (key, keyTarget, subs, update, tree, treeKey, stamp, rTree, force) {
  if (keyTarget && keyTarget.val === null) {
    if ('$remove' in subs) {
      diff(keyTarget, subs, update, treeKey, stamp, rTree, force)
    }
    if ('val' in subs) {
    ...
    removeReference(key, keyTarget, subs, update, tree, treeKey, stamp, rTree, force)
  }
...
function removeReference (key, keyTarget, subs, update, tree, treeKey, stamp, rTree, force) {
  if ('$remove' in subs) {
    diff(keyTarget, subs, update, treeKey, stamp, rTree, force)
  }
  if ('val' in subs) {
